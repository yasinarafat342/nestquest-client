    ...
    <nav className="bg-white/95 backdrop-blur-md sticky top-0 z-50 shadow-sm border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2 font-display font-bold text-xl text-dark-900">
            <FiHome className="text-primary-600 text-2xl" />
            <span>Nest<span className="text-primary-600">Quest</span></span>
          </Link>
        ...
        {menuOpen && (
          <div className="md:hidden py-4 border-t border-gray-100 space-y-3">
            <NavLink to="/" className="block px-4 py-2 text-gray-700 hover:text-primary-600" onClick={() => setMenuOpen(false)}>Home</NavLink>
            <NavLink to="/properties" className="block px-4 py-2 text-gray-700 hover:text-primary-600" onClick={() => setMenuOpen(false)}>All Properties</NavLink>
            {user && (
              <NavLink to={getDashboardLink()} className="block px-4 py-2 text-gray-700 hover:text-primary-600" onClick={() => setMenuOpen(false)}>Dashboard</NavLink>
            )}
            {user ? (
              <button onClick={() => { handleLogout(); setMenuOpen(false); }} className="w-full text-left px-4 py-2 text-red-600">Logout</button>
            ) : (
              <>
                <Link to="/login" className="block px-4 py-2 text-gray-700" onClick={() => setMenuOpen(false)}>Login</Link>
                <Link to="/register" className="block px-4 py-2 text-primary-600 font-semibold" onClick={() => setMenuOpen(false)}>Register</Link>
              </>
            )}
          </div>
        )}
