...
import { FiUser, FiMail, FiImage, FiEdit2, FiShield } from "react-icons/fi";
      ...
      <div className="card p-8">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="relative">
            <img
              src={form.photo || `https://ui-avatars.com/api/?name=${user?.name}&size=100&background=0284c7&color=fff`}
              alt={user?.name}
          ...
          <p className="font-display font-bold text-xl text-dark-900 mt-4">{user?.name}</p>
          <span className={`text-xs font-semibold px-3 py-1 rounded-full capitalize mt-2 ${roleColor[user?.role] || "bg-gray-100 text-gray-700"}`}>
        ...
        {!editing ? (
          <>
            <div className="space-y-3 mb-6">
              ...
              <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
                <FiShield className="text-primary-500 shrink-0" />
                <div>
                  <p className="text-xs text-gray-400">Account Role</p>
                  <p className="font-medium text-dark-900 capitalize">{user?.role}</p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {form.photo && (
                <img src={form.photo} alt="preview" className="w-16 h-16 rounded-full object-cover mt-2 border-2 border-primary-200" />
              )}
            </div>
            <div className="flex gap-3 pt-2">
              <button onClick={() => { setEditing(false); setForm({ name: user?.name, photo: user?.photo || "" }); }}
